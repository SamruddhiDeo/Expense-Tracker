...
<?php
    if($_SERVER['REQUEST_METHOD'] == "GET" && isset($_GET['delete'])){
     $id = $_GET["delete"];
        $sql = "DELETE FROM income WHERE `income`.`income_id` = $id;";
...
<body>
   ...
   ?>
    <div class="container">
        <h2 class="text-center my-2">Income Transactions</h2>
        <div class="transactionbox">
            <?php  
                     $username = $_SESSION['username'];
                  $finisheddates = array(); 
                  $sqld = "SELECT * FROM `income` WHERE username = '$username' ";
                  ...
                  $sql = "SELECT * FROM `income` WHERE date = '$value' AND username = '$username';";
                  $result = mysqli_query($conn,$sql);
          ...
          if($num > 0){
            while($row = mysqli_fetch_assoc($result)){
                ...
                <td><a href="incometransactions.php?delete='. $row['income_id'] .'"><button id="" class="delete">Delete</button></a></td>
            </tr> ';
          $no++;
                }
          }
